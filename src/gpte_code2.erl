...
-spec file_mode(chat_gpte:chat()) -> {klsn:binstr(), chat_gpte:chat()}.
file_mode(Chat0) ->
    ...
    {Res30, Chat30} = chat_gpte:ask(<<"Please respond in JSON format.
        ...
        is_done: Set true to finish or if there are nothing to do.
    ...
    Loop = fun
        Self({_Res, Chat}, 0) ->
            chat_gpte:ask(<<"Too many file editing request. Aborted. Your changes have been rollbacked. Exit file editing mode. Entered interactive mode. Your response will be displayed to the user so stop responding in JSON format. Tell the user that you could not apply the changes.">>, chat_gpte:delete_response_format(Chat));
        Self({Res, Chat}, N) ->
            case gpte_schema:schema(FileResp, Res) of
                #{ read := Read } when length(Read) > 0 ->
                    Self(read_file(Read, Chat), N-1);
                #{ write := Write } when length(Write) > 0 ->
                    Self(write_file(Write, Chat), N-1);
                #{ is_done := true } ->
                    chat_gpte:ask(<<"Exit file editing mode. Entered interactive mode. Your response will be displayed to the user so stop responding in JSON format. Simply explain how the file editing or reading went. Try not to print the entier content. Showing the diff may be a better idea.">>, chat_gpte:delete_response_format(Chat))
            end
    end,
    Loop({Res30, Chat30}, 10).

read_file(Read, Chat) ->
    error_logger:info_msg("read: ~p~n", [Read]),
    {ok, Cwd} = file:get_cwd(),
    Text = try
...
write_file(Write, Chat) ->
    error_logger:info_msg("write: ~p~n", [Write]),
    {ok, Cwd} = file:get_cwd(),
    Text = try
